...
def get_loss_function(cfg):
    if cfg['model_type'] == 'transformer_encoder_regression':
        return F.mse_loss
...
def compute_loss(loss_function, output, target, cfg):
    if cfg['model_type'] == 'evidence':
        ...
        return F.cross_entropy(output, target.long().to(cfg['device']))
